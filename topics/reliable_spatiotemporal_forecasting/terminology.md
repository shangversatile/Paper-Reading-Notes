...
| shared weights | Shared weights are trainable parameters reused across multiple positions, nodes, or structural components. Their gradients accumulate over all usages before each parameter update. | Candidate |
| fixed graph operator | A fixed graph operator is a graph-derived matrix such as an adjacency matrix, degree matrix, graph Laplacian, or rescaled Laplacian that defines propagation geometry but is not updated by gradient descent in the original ChebNet setting. | Candidate |
| gradient accumulation for shared parameters | Gradient accumulation for shared parameters means that every use of the same parameter in the forward computation contributes to the same total gradient during backpropagation. | Candidate |
...
| permutation equivariance | Permutation equivariance means that reindexing graph nodes should only reindex the model output, not change the model's underlying computation. | Candidate |
